...
namespace dmzx\buttonindex\includes;
	...
	*/
	...
	public function check($return_version = false)
	{
		...
		if ($allow_url_fopen)
		{
			...
			$template_data = array(
				'BUTTONINDEX_AUTHOR'				=> $this->version_data['author'],
				'BUTTONINDEX_CURRENT_VERSION'		=> $this->current_version,
				'BUTTONINDEX_UP_TO_DATE'			=> sprintf((!$version_up_to_date) ? $this->user->lang['BUTTONINDEX_NOT_UP_TO_DATE'] : $this->user->lang['BUTTONINDEX_UP_TO_DATE'], $this->version_data['title']),
				'BUTTONINDEX_S_UP_TO_DATE'			=> $version_up_to_date,
				'BUTTONINDEX_U_AUTHOR'				=> 'http://www.phpbb.com/community/memberlist.php?mode=viewprofile&un=' . $this->version_data['author'],
				'BUTTONINDEX_TITLE'					=> (string) $this->version_data['title'],
				'BUTTONINDEX_LATEST_VERSION'		=> $this->current_version,
			);
			...
			if (!$version_up_to_date)
			{
				...
				$template_data = array_merge($template_data, array(
					'BUTTONINDEX_ANNOUNCEMENT'		=> (string) $updates['announcement'],
					'BUTTONINDEX_DOWNLOAD'			=> (string) $updates['download'],
					'BUTTONINDEX_LATEST_VERSION'	=> $updates['current'],
				));
			}
